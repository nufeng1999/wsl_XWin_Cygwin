...
if [ $ProcNumber -le 0 ];then
	...
	#/usr/local/xeus-cling/bin/jupyter-lab  --ip=0.0.0.0 --allow-root --autoreload > /dev/null 2>&1 &
	jupyter-lab  --ip=0.0.0.0 --allow-root --autoreload --ServerApp.log_datefmt='%Y-%m-%d %H:%M:%S' > /tmp/jupyter.log 2>&1 &
fi
